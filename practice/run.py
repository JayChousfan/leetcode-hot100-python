...
def main() -> None:
    ...
    for filename, problem_id, title in PROBLEMS:
        result = subprocess.run(
            [sys.executable, str(practice_dir / filename)],
            env=environment,
            capture_output=True,
            text=True,
            encoding="utf-8",
        ...
        if result.returncode != 0 and "NameError" in result.stderr:
            print(f"⏳ LeetCode {problem_id}. {title}: NameError（等待填写）")
        else:
            print(f"⚠️ LeetCode {problem_id}. {title}: 未按预期报 NameError")
            unexpected.append(filename)

    if unexpected:
        print(f"\n异常文件: {', '.join(unexpected)}")
        raise SystemExit(1)

    print(f"\n✅ 全部 {len(PROBLEMS)} 个练习文件均等待填写")
